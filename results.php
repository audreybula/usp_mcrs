...
$header = get_string('search_results', 'block_usp_mcrs');
...
$PAGE->set_url('/blocks/usp_mcrs/results.php');
...
if (!$data = data_submitted()) {
    redirect(new moodle_url('/blocks/usp_mcrs/index.php'));
}
...
$criteria = array(
    ...
    get_string('course_id', 'block_usp_mcrs') => 'co.idnumber',
    get_string('category') => 'cat.name'
);

// Set up the search operators.
$operators = array(
    get_string('is', 'block_usp_mcrs') => 'IN',
    get_string('is_not', 'block_usp_mcrs') => 'NOT IN',
    get_string('contains', 'block_usp_mcrs') => 'LIKE',
    get_string('does_not_contain', 'block_usp_mcrs') => 'NOT LIKE'
);
...
echo html_writer::link('#', get_string('toggle_all', 'block_usp_mcrs'), array('class' => 'usp_mcrs toggle_link'));
echo '    <input type = "submit" value = "' . get_string('backup_button', 'block_usp_mcrs') . '"/>';
echo '</form>';
